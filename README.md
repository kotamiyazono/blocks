...
**どこに置くかはアプリが決めません。** 手番が回ってきただけでは盤には何も出ません。
ピースを選ぶか帯に触れると、その駒が盤の中央に現れます。中央は普通は置けない場所なので、
そこから帯や盤の上をなぞって自分で置き場所を決めます。
置ける場所を先回りして提示すると、手番が来た瞬間から確定できる状態になり、
考えずに「置く」を押すだけで進んでしまうためです。「置く」は置ける位置に来たときだけ押せます。
打った手は戻せません。実物と同じで、盤の上のものがそのまま今の状態です。

| したいこと | 操作 |
| --- | --- |
| ピースを選ぶ | カルーセルを横に流す／ピースをタップ。選んだ駒は盤の中央に出ます |
| 置く場所を決める | 帯をなぞる（指で駒を隠さずに動かせます）／盤を直接タップ・なぞる |
| 回転 | 駒か帯をタップ（回転 4 通り → 反転した 4 通り、と順に一巡します） |
| 反転 | 駒か帯を長押し |
| 確定 | 「置く」 |

PC では矢印キーで移動、`R` 回転、`F` 反転、`Enter` 決定、`Esc` 選択解除。
